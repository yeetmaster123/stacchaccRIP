...
void auto_revolver(SDK::CUserCmd* cmd) {
	...
	auto weapon = reinterpret_cast<SDK::CBaseWeapon*>(INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer())->GetActiveWeaponIndex());
	...
	for (auto players : Entities)
	{
		...
		if (weapon->GetItemDefenitionIndex() == SDK::WEAPON_AWP || weapon->GetItemDefenitionIndex() == SDK::WEAPON_SSG08 ||
			...
			if (!local_player->GetIsScoped()) {
				if (SETTINGS::settings.autoscope)
	...
	if (custom_hitbones[0].bselected)
		...
		if (custom_hitbones[8].bselected)
			...
			HITBOX_RIGHT_THIGH,
				...
				HITBOX_LEFT_FOOT,
				*/
	if (SETTINGS::settings.multiHitboxes[1])//head
		return SDK::HitboxList::HITBOX_HEAD;
	if (SETTINGS::settings.multiHitboxes[2])//neck
		return SDK::HitboxList::HITBOX_NECK;
	if (SETTINGS::settings.multiHitboxes[3])//thorax
		return SDK::HitboxList::HITBOX_THORAX;
	if (SETTINGS::settings.multiHitboxes[4])//chest
		return SDK::HitboxList::HITBOX_UPPER_CHEST, SDK::HitboxList::HITBOX_CHEST;
	if (SETTINGS::settings.multiHitboxes[5])//stomach
		return SDK::HitboxList::HITBOX_BODY;
	if (SETTINGS::settings.multiHitboxes[6])//pelvis
		return SDK::HitboxList::HITBOX_PELVIS;
	if (SETTINGS::settings.multiHitboxes[7])//Left arm
		return SDK::HitboxList::HITBOX_LEFT_UPPER_ARM, SDK::HitboxList::HITBOX_LEFT_FOREARM;
	if (SETTINGS::settings.multiHitboxes[8])//Right arm
		return SDK::HitboxList::HITBOX_RIGHT_UPPER_ARM, SDK::HitboxList::HITBOX_RIGHT_FOREARM;
	if (SETTINGS::settings.multiHitboxes[9])//Left hand
		return SDK::HitboxList::HITBOX_LEFT_HAND;
	if (SETTINGS::settings.multiHitboxes[10])//right hand
		return SDK::HitboxList::HITBOX_RIGHT_HAND;
	if (SETTINGS::settings.multiHitboxes[11])//Left thigh
		return SDK::HitboxList::HITBOX_RIGHT_THIGH;
	if (SETTINGS::settings.multiHitboxes[12])//Right thigh
		return SDK::HitboxList::HITBOX_LEFT_THIGH;
	if (SETTINGS::settings.multiHitboxes[13])//left shin
		return SDK::HitboxList::HITBOX_LEFT_CALF;
	if (SETTINGS::settings.multiHitboxes[14])//right shin
		return SDK::HitboxList::HITBOX_RIGHT_CALF;
	if (SETTINGS::settings.multiHitboxes[15])//left foot
		return SDK::HitboxList::HITBOX_LEFT_FOOT;
	if (SETTINGS::settings.multiHitboxes[16])//right foot
		return SDK::HitboxList::HITBOX_RIGHT_FOOT;
...
Vector CAimbot::multipoint(SDK::CBaseEntity* entity, int option)
{
	...
	/*if (SETTINGS::settings.smartbaim) {
		...
		auto weapon = reinterpret_cast<SDK::CBaseWeapon*>(INTERFACES::ClientEntityList->GetClientEntity(local_player->GetActiveWeaponIndex()));
		if (SETTINGS::settings.baimifawp && weapon->GetItemDefenitionIndex() == SDK::WEAPON_AWP) //men why rageawp reeeeee
		{

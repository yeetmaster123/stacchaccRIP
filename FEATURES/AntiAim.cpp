...
void ZAnti(SDK::CUserCmd* cmd)
{
	...
	cmd->viewangles.y += angleAdditive - maxRange / 2.f + std::fmodf(INTERFACES::Globals->curtime * 150, maxRange);
}

void CAntiAim::freestand(SDK::CUserCmd* cmd) //setting up aa
{
	auto local_player = INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer());
	//float reverse_delta = SETTINGS::settings.delta_val * (-1);
	...
	if (!(local_player->GetFlags() & FL_ONGROUND)) {
		...
		if (next_lby_update(cmd->viewangles.y, cmd)) {
			cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) + SETTINGS::settings.delta_val;
			global::breaking_lby = true;
		}
		//else if ((local_update + 0.05f) <= 0 && SETTINGS::settings.cradle)
		//	cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) + reverse_delta;
		else {
			cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) - 35.0f / 2.f + std::fmodf(INTERFACES::Globals->curtime * 150, 35.0f);
			global::breaking_lby = false;
		}
	}
}
...
void CAntiAim::do_antiaim(SDK::CUserCmd* cmd) {
	...
	if ((local_player->GetVelocity().Length2D() < 80) && !(cmd->buttons & IN_JUMP) && !fake_walk) {
		...
		if (!global::should_send_packet && net_channel->m_nChokedPackets == 0)
			...
			case 1: {
				if (next_lby_update(cmd->viewangles.y, cmd)) {
					cmd->viewangles.y - 90 + SETTINGS::settings.delta_val;
					global::breaking_lby = true;
				}
				else 
					cmd->viewangles.y += 180.f + SETTINGS::settings.aa_realadditive_val;

				if (SETTINGS::settings.backjitter)
					cmd->viewangles.y += 180 + ((rand() % 15) - (15 * 0.5f)) + SETTINGS::settings.aa_realadditive_val;
			} break;
			...
			case 3: {
				if (next_lby_update(cmd->viewangles.y, cmd)) {
					...
					cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) - (SETTINGS::settings.spinangle + 1) / 2.f + std::fmodf(INTERFACES::Globals->curtime * ((SETTINGS::settings.spinspeed1 * 5) + 1), (SETTINGS::settings.spinangle + 1));
					global::breaking_lby = false;
				}
			} break;
			}
		...
		case 1: {
			if (next_lby_update(cmd->viewangles.y, cmd)) {
				cmd->viewangles.y += SETTINGS::settings.delta1_val;
				global::breaking_lby = true;
			}
			else
				cmd->viewangles.y += 180.f + SETTINGS::settings.aa_realadditive1_val;
			if (SETTINGS::settings.backjitter1)
				cmd->viewangles.y += 180 + ((rand() % 15) - (15 * 0.5f)) + SETTINGS::settings.aa_realadditive_val;
		} break;
		case 2: {
			if (flipaa)
		...
		{
			cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) - (SETTINGS::settings.spinangle1 + 1) / 2.f + std::fmodf(INTERFACES::Globals->curtime * ((SETTINGS::settings.spinspeed1 * 5) + 1), (SETTINGS::settings.spinangle1 + 1));
			//cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y);
		//	global::breaking_lby = false;
		} break;
		//}
	...
	else if (!fake_walk)
	{
		...
		case 1: {
			if (next_lby_update(cmd->viewangles.y, cmd)) {
				cmd->viewangles.y += SETTINGS::settings.delta2_val;
				global::breaking_lby = true;
			}
			else
				cmd->viewangles.y += 180.f + SETTINGS::settings.aa_realadditive2_val;

			if (SETTINGS::settings.backjitter2)
				cmd->viewangles.y += 180 + ((rand() % 15) - (15 * 0.5f)) + SETTINGS::settings.aa_realadditive_val;
		} break;
		case 2: {
			if (flipaa)
		...
		case 3: //freestand(cmd); break;
		{
			cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) - SETTINGS::settings.spinangle2 / 2.f + std::fmodf(INTERFACES::Globals->curtime * (SETTINGS::settings.spinspeed2 * 5), SETTINGS::settings.spinangle2);
			//global::breaking_lby = false;
		} break;
		//	}
		}
	...
	{
		switch (SETTINGS::settings.aa_pitch3_type)
		{
		case 0: break;
		case 1: cmd->viewangles.x = 85.f; break;
		case 2: cmd->viewangles.x = -179.f; break;
		...
		if (global::should_send_packet && SETTINGS::settings.desync_yaw)
			...
			switch (SETTINGS::settings.aa_real3_type)
			{
			case 0: break;
			case 1: {
				if (next_lby_update(cmd->viewangles.y, cmd))
					cmd->viewangles.y += SETTINGS::settings.delta3_val;

				else
					cmd->viewangles.y += 180.f + SETTINGS::settings.aa_realadditive3_val;

				if (SETTINGS::settings.backjitter3)
					cmd->viewangles.y += 180 + ((rand() % 15) - (15 * 0.5f)) + SETTINGS::settings.aa_realadditive_val;
			} break;
			case 2: {
				if (flipaa)
				{
					if (next_lby_update(cmd->viewangles.y + SETTINGS::settings.delta3_val, cmd))
					{
						global::breaking_lby = true;
						cmd->viewangles.y = last_real + SETTINGS::settings.delta3_val;
					}
					else
					{
						global::breaking_lby = false;
						cmd->viewangles.y -= 90 + SETTINGS::settings.aa_realadditive3_val;
						last_real = cmd->viewangles.y;
					}
				}
				else
			...
			{
				if (next_lby_update(cmd->viewangles.y, cmd)) {
					cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) + SETTINGS::settings.delta3_val;
					global::breaking_lby = true;
				}
				else {
					cmd->viewangles.y = detectdatwallbitch(cmd->viewangles.y) - SETTINGS::settings.spinangle3 / 2.f + std::fmodf(INTERFACES::Globals->curtime * (SETTINGS::settings.spinspeed3 * 5), SETTINGS::settings.spinangle3);
					global::breaking_lby = false;
				}
			} break;
			//	}

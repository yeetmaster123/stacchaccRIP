...
namespace Menuxd
{
	...
	void DrawCornierBoxFastAlpha(float x, float y, float height, float width, D3DCOLOR COLOR, float alpha, IDirect3DDevice9* pDevice)
	{
		...
		switch (SETTINGS::settings.Visuals_BoxType)
		{
		case 0:
		{
...
bool D9Visuals::Filter(SDK::CBaseEntity *entity, bool& IsVis)
{
	...
	if (entity->GetTeam() == local_player->GetTeam() && !SETTINGS::settings.Visuals_EspTeam) return true;
	if (entity->GetTeam() != 2 && entity->GetTeam() != 3) return true;

	IsVis = (entity->IsVisible(8) && !entity->GetIsDormant());
	if (SETTINGS::settings.Visuals_VisableOnly && !IsVis)return true;
	return false;
}
...
void D9Visuals::DrawHealth(SDK::CBaseEntity *entity, IDirect3DDevice9* pDevice)
{
	...
	bool enemy = entity->GetTeam() != local_player->GetTeam() || SETTINGS::settings.Visuals_EspTeam;

	if (!enemy && !SETTINGS::settings.Visuals_EspTeam)
		return;
	...
	if (GetPlayerBox(entity, x, y, width, height))
	{
		...
		if (SETTINGS::settings.Visuals_HealthBarType == 1)
		{
			for (int i = 1; i < 4; i++)
...
void D9Visuals::DrawBox(SDK::CBaseEntity *entity, int cType, IDirect3DDevice9* pDevice)
{
	...
	if (GetPlayerBox(entity, x, y, width, height))
	{
		...
		if (IsVisible)
		{
			if (entity->GetTeam() == 3)
			{
				boxColor = D3DCOLOR_ARGB(PlayerReadyAlpha(entity), int(SETTINGS::settings.BoxColorPickCTVIS[0] * 255), int(SETTINGS::settings.BoxColorPickCTVIS[1] * 255), int(SETTINGS::settings.BoxColorPickCTVIS[2] * 255));
			}
			else
			{
				boxColor = D3DCOLOR_ARGB(PlayerReadyAlpha(entity), int(SETTINGS::settings.BoxColorPickTVIS[0] * 255), int(SETTINGS::settings.BoxColorPickTVIS[1] * 255), int(SETTINGS::settings.BoxColorPickTVIS[2] * 255));
			}
		}
		else
		{
			if (entity->GetTeam() == 3)
			{
				boxColor = D3DCOLOR_ARGB(PlayerReadyAlpha(entity), int(SETTINGS::settings.BoxColorPickCTINVIS[0] * 255), int(SETTINGS::settings.BoxColorPickCTINVIS[1] * 255), int(SETTINGS::settings.BoxColorPickCTINVIS[2] * 255));
			}
			else
			{
				boxColor = D3DCOLOR_ARGB(PlayerReadyAlpha(entity), int(SETTINGS::settings.BoxColorPickTINVIS[0] * 255), int(SETTINGS::settings.BoxColorPickTINVIS[1] * 255), int(SETTINGS::settings.BoxColorPickTINVIS[2] * 255));
			}
		}
		...
		if (SETTINGS::settings.Visuals_BoxEnabled)
		{
...
_x = x + width + padd;
_y = y;
if (SETTINGS::settings.Visuals_AimLines)
{
...
if (SETTINGS::settings.Visuals_Weapons && SETTINGS::settings.Visuals_WeaponsType == 0)
{
...
if (SETTINGS::settings.Visuals_ArmorBar)
{
...
if (SETTINGS::settings.Visuals_AmmoESP && SETTINGS::settings.Visuals_AmmoESPType == 1)
{
...
if (SETTINGS::settings.Visuals_Weapons && SETTINGS::settings.Visuals_WeaponsType == 1)
{
...
void D9Visuals::Render(IDirect3DDevice9* pDevice)
{
	...
	/*
	if (SETTINGS::settings.Visuals_DrawLinesAA)
	{
	DrawLinesAA();
	}
	*/
	...
	for (int i = 1; i < INTERFACES::Engine->GetMaxClients(); i++)
	{
		...
		if (!Filter(entity, Visable))
		{
			...
			if (SETTINGS::settings.Visuals_Name) DrawName(entity);

			if (SETTINGS::settings.Visuals_BoxEnabled) DrawBox(entity, typecolor, pDevice);
			if (SETTINGS::settings.Visuals_HealthBar) DrawHealth(entity, pDevice);
	...
	if (SETTINGS::settings.Visuals_C4 && strstr(pEntity->GetClientClass()->m_pNetworkName, "CPlantedC4"))
	{
	...
	if (SETTINGS::settings.Visuals_DroppedWeapons && pEntity->IsDefusing())
	{
...
namespace DamageESP
{
	...
	void HandleGameEvent(SDK::IGameEvent* pEvent)
	{
		...
		if (!SETTINGS::settings.Visuals_DamageESP || !(INTERFACES::Engine->IsInGame() && INTERFACES::Engine->IsConnected() && local_player))
			return;
	...
	void Draw()
	{
		if (!SETTINGS::settings.Visuals_DamageESP)
			return;
		...
		for (int i = 0; i < MAX_FLOATING_TEXTS; i++)
		{
			...
			if (Menuxd::WorldToScreen(txt->hitPosition, screen))
			{
				...
				Menuxd::DrawStringWithFont(Menuxd::fntVerdana10, screen.x - width / 2, screen.y - 11, D3DCOLOR_ARGB(int(SETTINGS::settings.DamageESPColor[3] * 255), int(SETTINGS::settings.DamageESPColor[0] * 255), int(SETTINGS::settings.DamageESPColor[1] * 255), int(SETTINGS::settings.DamageESPColor[2] * 255)), msg);
			}
		}
	}
};

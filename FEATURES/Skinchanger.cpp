		...
		if (pEntity)
		{
			...
			if (SETTINGS::settings.skinenabled)
			{
				...
				if (pEntity->GetClientClass()->m_ClassID == 105)
				{
					...
					else if (Model == 14) // minecaft
					{
						//SDK::CBaseWeapon* pWeapon = (SDK::CBaseWeapon*)INTERFACES::ClientEntityList->GetClientEntityFromHandle(pLocal->GetActiveWeapon());
						//int weapon = pWeapon->GetItemDefenitionIndex();

						int index = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_minecraft_pickaxe.mdl");

						visuals->PrecacheModel("models/weapons/v_minecraft_pickaxe.mdl");
						if (pWeapon->is_knife())
							pWeapon->SetModelIndex(INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_minecraft_pickaxe.mdl"));

						*pWeapon->ModelIndex() = index; // m_nModelIndex
						*pWeapon->ViewModelIndex() = index;
						if (worldmodel) *worldmodel->ModelIndex() = index + 1;
					...
					if (SETTINGS::settings.m4a4model == 1)
					{
						int index = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_ak47royalguard.mdl");

						visuals->PrecacheModel("models/weapons/v_ak47royalguard.mdl");
						if (pWeapon->GetItemDefenitionIndex() == SDK::definition_index::WEAPON_AK47)
							pWeapon->SetModelIndex(INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_ak47royalguard.mdl"));
						...
						*pWeapon->ViewModelIndex() = index;
						...
						killIcons.clear();
						killIcons["rif_ak47"] = "materials/panorama/images/hud/deathnotice/icon-mp5sd.png";

					}
					else if (SETTINGS::settings.m4a4model == 2)
					{
						int index = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_ak47_new.mdl");

						visuals->PrecacheModel("models/weapons/v_rif_ak47_new.mdl");
						if (pWeapon->GetItemDefenitionIndex() == SDK::definition_index::WEAPON_AK47)
							pWeapon->SetModelIndex(INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_ak47_new.mdl"));
					...
					if (SETTINGS::settings.m4a4smodel == 1)
					{
						int index = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_m4a1_s_new.mdl");

						visuals->PrecacheModel("models/weapons/v_rif_m4a1_s_new.mdl");
						if (pWeapon->GetItemDefenitionIndex() == SDK::definition_index::WEAPON_M4A1_SILENCER)
							pWeapon->SetModelIndex(INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_m4a1_s_new.mdl"));

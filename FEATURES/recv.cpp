...
#include "../FEATURES/StickerChanger.h"
...
void Hooked_RecvProxy_Viewmodel(SDK::CRecvProxyData *pData, void *pStruct, void *pOut)
{
	...
	SDK::CBaseWeapon* pWeapon = (SDK::CBaseWeapon*)pLocal;
	...
	int PICKAXE = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_minecraft_pickaxe.mdl");

	int m4a1sold = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_m4a1_s.mdl");
	int m4a1snew = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_m4a1_s_new.mdl");
	int ak47old = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_ak47.mdl");
	int ak47new = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_ak47royalguard.mdl");
	int ak472 = INTERFACES::ModelInfo->GetModelIndex("models/weapons/v_rif_ak47_new.mdl");
	...
	if (SETTINGS::settings.skinenabled && pLocal)
	{
			...
			pData->m_Value.m_Int == Talon))
		{
			...
			if (SETTINGS::settings.Knife == 0)
				...
				pData->m_Value.m_Int = Talon;
			else if (SETTINGS::settings.Knife == 14)
				pData->m_Value.m_Int = PICKAXE;
		}

		if (pLocal->IsAlive() && (
		pData->m_Value.m_Int == ak47old))
		{
		if (SETTINGS::settings.m4a4model == 1)
			pData->m_Value.m_Int = ak47new;
		else if (SETTINGS::settings.m4a4model == 2)
			pData->m_Value.m_Int = ak472;
		}

		if (pLocal->IsAlive() && (
			pData->m_Value.m_Int == m4a1sold))
		{
			if (SETTINGS::settings.m4a4smodel == 1)
				pData->m_Value.m_Int = m4a1snew;
		}
	}
...
void SetViewModelSequence2(const SDK::CRecvProxyData *pDataConst, void *pStruct, void *pOut)
{
	...
	if (pViewModel)
	{
		...
		if (pOwner && pOwner->GetIndex() == INTERFACES::Engine->GetLocalPlayer())
		{
			...
			/*else if (szModel == "models/weapons/v_minecraft_pickaxe.mdl")
			{

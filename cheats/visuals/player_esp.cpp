...
void cplayer_esp::paint_traverse() {
	...
	for (int i = 1; i < INTERFACES::Globals->maxclients; i++) {
		...
		if (SETTINGS::settings.fov_bool) pov_arrow(entity);
		if (SETTINGS::settings.name_bool) draw_name(entity);
		if (SETTINGS::settings.weapon_esp) draw_weapon(entity);

		if (SETTINGS::settings.skeleton) {
...
void cplayer_esp::draw_name(SDK::CBaseEntity* m_entity, Box box) {
	...
	if (INTERFACES::Engine->GetPlayerInfo(m_entity->GetIndex(), &player_info)) {
		...
		auto color = m_entity->GetIsDormant() ? CColor(130, 130, 130, 130) : SETTINGS::settings.name_col;
...
void cplayer_esp::draw_weapon(SDK::CBaseEntity* m_entity, Box box) {
	...
	SDK::player_info_t player_info;
	INTERFACES::Engine->GetPlayerInfo(m_entity->GetIndex(), &player_info);

	if (!SETTINGS::settings.weapons)
		return;

	int bottom_pos = 0;

	auto weapon = reinterpret_cast<SDK::CBaseWeapon*>(INTERFACES::ClientEntityList->GetClientEntity(m_entity->GetActiveWeaponIndex()));
	...
	bool drawing_ammo_bar = SETTINGS::settings.ammo_bool == 1;
	bool drawing_lby_bar = SETTINGS::settings.lby_bar;

	if (weapon->is_non_aim() || !SETTINGS::settings.lby_bar || player_info.fakeplayer)
	...
	auto color = m_entity->GetIsDormant() ? CColor(130, 130, 130, 130) : SETTINGS::settings.weapon_col;
	color.SetAlpha(255 * esp_alpha_fade[m_entity->GetIndex()]);
...
void cplayer_esp::draw_flags(SDK::CBaseEntity* e, Box box) {
	...
	SDK::player_info_t player_info;
	INTERFACES::Engine->GetPlayerInfo(e->GetIndex(), &player_info);
...
void cplayer_esp::draw_lbybar(SDK::CBaseEntity* m_entity, Box box) {
	...
	if (player_info.fakeplayer || !SETTINGS::settings.lbyenable)
		return;

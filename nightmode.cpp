...
void nightmode::apply() {
	...
	if (!sv_skyname) {
		sv_skyname = INTERFACES::cvar->FindVar("sv_skyname");
	...
	SDK::ConVar* specific_prop = INTERFACES::cvar->FindVar("r_drawspecificstaticprop");
	...
	for (SDK::MaterialHandle_t i = INTERFACES::MaterialSystem->FirstMaterial(); i != INTERFACES::MaterialSystem->InvalidMaterial(); i = INTERFACES::MaterialSystem->NextMaterial(i)) {
		SDK::IMaterial* material = INTERFACES::MaterialSystem->GetMaterial(i);
...
void nightmode::remove() {
	...
	executed = false;
	SETTINGS::settings.night_bool = false;
